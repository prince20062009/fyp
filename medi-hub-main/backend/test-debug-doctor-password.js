...
// Test debug doctor password
const testDebugDoctorPassword = async () => {
    try {
        const doctor = await Doctor.findOne({ email: 'dr.debug.doctor@example.com' }).select('+password');
        if (doctor) {
